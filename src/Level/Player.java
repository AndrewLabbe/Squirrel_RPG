...
public abstract class Player extends GameObject {
	...
	protected void playerWalking() {
		...
		// if walk right key is pressed, move player to the right
		// Checks to see if main character has reach map bounds
		else if (Keyboard.isKeyDown(MOVE_RIGHT_KEY) && Math.round(getX()) < map.getWidthPixels() - 55
				&& !Keyboard.isKeyDown(SPEED_KEY)) {
		...
		// Walk faster
			...
			currentWalkingYDirection = Direction.UP;
			lastWalkingYDirection = Direction.UP;
		} else if (Keyboard.isKeyDown(MOVE_DOWN_KEY) && Math.round(getY()) < map.getHeightPixels() - 60
				&& !Keyboard.isKeyDown(SPEED_KEY)) {
			moveAmountY += walkSpeed;
			currentWalkingYDirection = Direction.DOWN;
			lastWalkingYDirection = Direction.DOWN;
	...
	public void fireBullet() {
		if (fireDelay.isTimeUp()) {
			...
			// Sets spawn point and direction of projectile depending on which was character
			// is facing
			// If moving in horizontally or vertically only set movement to 1 but if
			// traveling both vertically
			// and horizontally at the same time use .71 and .71 because length of that
			// vector will equal 1
			// This eliminates the bug where firing diagonally moves faster
			// If player is moving left
			if (getCurrentWalkingXDirection() == Direction.LEFT || getLastWalkingXDirection() == Direction.LEFT) {
			...
			// If player is moving right
			else if (getCurrentWalkingXDirection() == Direction.RIGHT
					|| getLastWalkingXDirection() == Direction.RIGHT) {
				projectileX = Math.round(getX()) + 40;
				directionX = 1;
				// If player is moving right and up
				if (getCurrentWalkingYDirection() == Direction.UP || getLastWalkingYDirection() == Direction.UP) {
